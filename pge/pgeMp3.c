...
static void ParseID3v2_2(pgeMp3Info *id3tag)
{
	...
	if(ID3v2() == 2)
	{
		...
		while(size != 0)
		{
			...
			{
				pgeFileSeek(pgeMp3Handle, 1, PGE_FILE_CUR);
				id3tag->artist = (char *)malloc(tag_length);
			...
			{
				pgeFileSeek(pgeMp3Handle, 1, PGE_FILE_CUR);
				id3tag->title = (char *)malloc(tag_length);
			...
			{
				pgeFileSeek(pgeMp3Handle, 1, PGE_FILE_CUR);
				id3tag->album = (char *)malloc(tag_length);
			...
			{
				pgeFileSeek(pgeMp3Handle, 1, PGE_FILE_CUR);
				id3tag->tracktext = (char *)malloc(tag_length);
			...
			{
				pgeFileSeek(pgeMp3Handle, 1, PGE_FILE_CUR);
				id3tag->year = (char *)malloc(tag_length);
			...
			else if(!strncmp("COM", tag, 3))
			{
				...
				id3tag->comment = (char *)malloc(tag_length - skip);
			...
			{
				pgeFileSeek(pgeMp3Handle, 1, PGE_FILE_CUR);
				id3tag->genretext = (char *)malloc(tag_length);
...
static int ParseID3v1(pgeMp3Info *id3tag)
{
    ...
    if(strstr(id3buffer, "TAG") != NULL)
	{
        ...
        if(id3tag->genrecode == NULL)
		{
			...
			id3tag->genrecode = (char *)malloc(2);
		...
		{
			id3tag->genretext = (char *)malloc(31);
...
static void freeMp3Info(pgeMp3Info *id3tag)
{
	...
	if(id3tag->genrecode)
		free(id3tag->genrecode);
		
	if(id3tag->genretext)
		free(id3tag->genretext);
		
	if(id3tag->tracktext)
		free(id3tag->tracktext);
