/*
 * $Id: OMPUtil.cpp 86 2010-01-03 04:12:17Z chaotic@luqmanrocks.co.cc $
 * 
...
void OMPUtil::loadMainUI() {
	    ...
	    // Load surface
		image = engine->sdlSurfaceFromPakFile("Resources.omle", "MainUI.png");
		
		// Error check
		if (image == NULL)
			engine->logMsg("MenuState: Unable to load SDL_Surface from Pak file. [MainUI][%s]", SDL_GetError());
					
		// Load surface into resource manager
		if (!ResourceManager::loadTextureFromSdlSurface("MainUI", image, NULL))
			engine->logMsg("MenuState: Unable to load surface into Resource Manager [MainUI].");
 ...
 */
void OMPUtil::loadCommonResources() {
	    ...
	    // Load surface
		image = engine->sdlSurfaceFromPakFile("Resources.omle", "BatteryIcons.png");
		
		// Error check
		if (image == NULL)
			engine->logMsg("MenuState: Unable to load SDL_Surface from Pak file. [BatteryIcons][%s]", SDL_GetError());
			
		// Load surface into resource manager [Battery Icons]
		if (!ResourceManager::loadTextureFromSdlSurface("BatteryIcons", image, NULL))
			engine->logMsg("MenuState: Unable to load surface into Resource Manager [BatteryIcons].");
	    ...
	    // Load surface
		image = engine->sdlSurfaceFromPakFile("Resources.omle", "LoadingIcons.png");
		
		// Error check
		if (image == NULL)
			engine->logMsg("MenuState: Unable to load SDL_Surface from Pak file. [LoadingIcons][%s]", SDL_GetError());

		// Load surface into resource manager [Loading Icons]
		if (!ResourceManager::loadTextureFromSdlSurface("LoadingIcons", image, NULL))
			engine->logMsg("MenuState: Unable to load surface into Resource Manager [LoadingIcons].");
	    ...
	    // Load font
	    OMText *font = new OMText("SansBold.ttf", 15);
	        
	    // Put font in Resource Manager
	    if (!ResourceManager::loadFont("SansBold15", font))
	    	engine->logMsg("MenuState: Unable to load font into Resource Manager [SansBold15].");
 ...
 */
void OMPUtil::drawBatteryIcon() {

	// Get Battery icons
	OMTexture texture = ResourceManager::getTexture("BatteryIcons");
 ...
 */
void OMPUtil::drawLoadingIcon(int x, int y, bool withBG) {
	...
	// Get texture	
	OMTexture texture = ResourceManager::getTexture("LoadingIcons");
